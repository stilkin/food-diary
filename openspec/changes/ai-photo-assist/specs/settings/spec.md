...
The system SHALL allow the user to enter and clear an OpenRouter API key for AI-assisted photo description.

#### Scenario: API key saved
- **WHEN** the user enters an API key and saves
- **THEN** the key SHALL be persisted in expo-secure-store

#### Scenario: API key displayed masked
- **WHEN** a saved API key exists
- **THEN** the settings field SHALL display it masked (password-style input)

#### Scenario: API key cleared
- **WHEN** the user clears the API key field and saves
- **THEN** the key SHALL be removed from expo-secure-store and AI assistance SHALL be disabled
